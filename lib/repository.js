...
function fetch(key, repo, commitish, fetchCallback) {
  ...
  if (repositoryExists && commitishExists) {
    // TODO We need to touch the files here (wait for cache.js)
    return fetchCallback(null, commitishPath);
  }
  ...
  async.series(
    [
      ...
      // d. Determine if we need to perform the fetch and checkout
      ...
      // e. Fetch all on repository so it is fully up-to-date (if applicable)
      function fetchAll(cb) {
        if (!fetchAndCheckout) { return cb(); }
        git.fetchAll(cb);
      },

      // f. Checkout the specific commitish (if applicable)
