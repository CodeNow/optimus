...
const log = require('./logger').child({ module: 'repository' })
...
/**
 * Testing debug for examining the repository fetch workflow.
 * DEBUG=optimus-test:repo npm test
 * @type {object}
 */
...
function getLock (path, cb) {
  if (!locks[path]) {
    log.trace('Spin-lock acquired: ' + path)
    locks[path] = true
    return cb()
  }
...
function freeLock (path) {
  log.trace('Spin-lock freed: ' + path)
  delete locks[path]
}
...
function fetch (key, repo, commitish, fetchCallback) {
  log.info('Fetching repository ' + repo + '#' + commitish)
  ...
  if (repositoryExists && commitishExists) {
    log.debug('Cache hit for ' + repo + '#' + commitish)
  ...
  if (!fs.existsSync(key)) {
    return fetchCallback(error.create(
      500, 'repository.fetch: Could not find ssh key', { key: key }
    ))
  }
  ...
  async.series(
    ...
    function finish (err) {
      // We have to free the locks here in case anything fails before it
      // reaches the last step (otherwise: DEADLOCK)
...
function fetchFromRemote (
  ...
  async.series([
    ...
    function createRepoDirectory (next) {
      ...
      childProcess.exec('mkdir -p ' + repositoryPath, function (err) {
        if (err) {
          err.data = { path: repositoryPath }
          return next(error.wrap(
            err, 500, 'repository.fetchFromRemote.createRepoDirectory'
          ))
        }
        next()
      })
    ...
    function cloneRepository (next) {
      ...
      git.clone(repo, repositoryPath, function (err) {
        if (err) {
          err.data = {
            repo: repo,
            path: repositoryPath
          ...
          return next(error.wrap(
            err, 502, 'repository.fetchFromRemote.createRepoDirectory'
          ))
        }
        next()
      })
    ...
    function createCommitishDir (next) {
      ...
      childProcess.exec(command, function (err) {
        if (err) {
          ...
          return next(error.wrap(
            err, 500, 'repository.fetchFromRemote.createCommitishDir'
          ))
        }
        next()
      })
    ...
    function compareSHAs (next) {
      ...
      git.getSHA(function (err, sha) {
        if (err) {
          err.data = { repo: repo, commitish: commitish }
          return next(error.wrap(
            err, 500, 'repository.fetchFromRemote.compareSHAs'
          ))
        }
        if (!sha || isEmpty(sha)) {
    ...
    function fetchAll (next) {
      ...
      git.fetchAll(function (err) {
        if (err) {
          err.data = { deployKey: key, repo: repo }
          return next(error.wrap(
            err, 502, 'repository.fetchFromRemote.fetchAll'
          ))
        }
        next()
      })
    ...
    function checkout (next) {
      ...
      git.checkout(commitish, function (err) {
        if (err) {
          err.data = { repo: repo, commitish: commitish }
          return next(error.wrap(
            err, 500, 'repository.fetchFromRemote.checkout'
          ))
        }
        next()
      })
    ...
    function removeDotGit (next) {
      ...
      childProcess.exec(removeCommand, function (err) {
        if (err) {
          err.data = {
            commitishPath: commitishPath,
          ...
          return next(error.wrap(
            err, 500, 'repository.fetchFromRemote.removeDotGit'
          ))
        }
        next()
      })
