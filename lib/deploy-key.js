...
var log = require('./logger').child({ module: 'deploy-key' });
    ...
    [
      function createCachePath(cb) {
        ...
        childProcess.exec('mkdir -p ' + cachePath, function (err) {
          if (err) {
            log.error(
              err,
              'Could not create deploy key cache entry: ' + cachePath
            );
          }
          cb(err);
        });
      ...
      function downloadKey(cb) {
        ...
        s3.getObject(object, function (err, data) {
          // TODO Need to return correct status code from this if it fails
          //      (502 if it freaks out, 404 if not found, etc.). This logic
          //      is most likely in API (look around)
          if (err) {
            log.error(err, 'Could not fetch object from S3');
            return cb(err);
          }
      ...
      function chmodKey(cb) {
        childProcess.exec('chmod 600 ' + sshKeyPath, function (err) {
          if (err) {
            log.error(
              err,
              'Could not change permissions on key: ' + sshKeyPath
            );
          }
          cb(err);
        });
...
function exec(keyPath, command, options, cb) {
  ...
  childProcess.exec(sshCommand, options, function (err) {
    if (err) {
      log.error(err, 'Unable to execute command: ' + sshCommand);
    }
    cb(err);
  });
}
