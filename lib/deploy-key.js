    ...
    return Promise.resolve()
      .then(() => {
        ...
        if (fs.existsSync(sshKeyPath)) {
          log.debug({ sshKeyPath: sshKeyPath }, 'SSH Key exists in Cache')
        ...
        return Promise.resolve()
          .then(function createCachePath () {
            if (fs.existsSync(cachePath)) {
              log.trace({ cachePath: cachePath }, 'Cache path exists')
              return
            }
            log.trace({ cachePath: cachePath }, 'Creating cache path')
          ...
          .then(function chmodKey () {
            log.trace({ sshKeyPath: sshKeyPath }, 'Setting ssh key permissions')
