 ...
 * @type {object}
 */
var cacheDebug = debug('optimus-test:cache');
...
function initialize(initCallback) {
  ...
  async.map(getCaches(), function (path, cb) {
    childProcess.exec('mkdir -p ' + path, function (err) {
      if (err) {
        log.fatal({ err: err, path: path }, 'Unable to initialize cache');
      }
      else {
        log.debug({ path: path }, 'Cache initialized');
      }
      cb(err);
    });
  }, initCallback);
...
function setPurgeInterval() {
  ...
  log.info({
    purgeInterval: purgeIntervalDuration
  }, 'Cache purge interval set');
}
...
function usage(cb) {
  ...
  async.map(
    ...
    function compile(err, caches) {
      if (err) {
        log.error({ err: err }, 'Unable to collect disk usage information');
        return cb(err);
      }
  ...
  async.map(
    ...
    function execute(path, cb) {
      ...
      childProcess.exec(purgeCommand, function (err) {
        if (err) {
          log.error({ err: err }, 'Purge failed');
        }
        else {
          log.debug({ path: path }, 'Cache purged');
        }
        cb(err);
      });
    },
