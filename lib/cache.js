...
class Cache {
   ...
   */
  unlock (path) {
    const lockPath = `${path}/.optimus_lock`
    const log = logger.child({ method: 'unlock', lockPath: lockPath })
    if (!fs.existsSync(lockPath)) {
      log.warn('Lock path does not exist')
      return Promise.resolve()
    }
    return childProcess.execFileAsync('rmdir', [lockPath])
  }
